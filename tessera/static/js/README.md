...
feature-focused modules. At build time the assembler (`tessera/core/assembler.py`)
reads them **in a fixed order**, concatenates them, and inlines the result into the
report's single
`<script>` block (see `MAIN_JS_MODULES` in `tessera/core/assembler.py`). There is
...
The solution is a **shared namespace**: one global object, `window.Tessera`
...
`state.js` **first** (creates `window.Tessera` and `T.state`), `boot.js` **last**
...
1. Create `js/<name>.js` using the standard wrapper:
   ...
   ```
2. Add `"<name>"` to `MAIN_JS_MODULES` in `tessera/core/assembler.py` (mind the
