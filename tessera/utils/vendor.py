"""
tessera.utils.vendor
====================
Resolves a declared :class:`~tessera.core.plugins.Plugin` into the concrete
``<script>`` / ``<link>`` assets the template should emit, honouring the chosen
loading mode (CDN vs bundled) and the vendor ``manifest.json``.

The :class:`~tessera.core.assembler.Assembler` owns all file IO (reading the
...
from tessera.utils.resource_loader import get_static
